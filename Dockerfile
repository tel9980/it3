...
RUN echo 'hxf' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/hxf.sh

RUN echo 'cd ' >>/hxf.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/hxf.sh

RUN echo 'cd /noVNC-1.2.0' >>/hxf.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/hxf.sh

RUN chmod 755 /hxf.sh
